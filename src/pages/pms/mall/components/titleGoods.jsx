...
export default class TitleGoods extends Component {
    ...
    render() {
        const { form: { getFieldDecorator }, moduleItem, afterSave, groupList } = this.props
        let groupOptions = groupList.map(group=>{
            return (
        ...
        return (
            <div className={Styles.editContent}>
                ...
                {moduleItem.code != '1100' && <FormItem label="首页预览个数" labelCol={{ span: 5 }} wrapperCol={{ span: 10 }} help='预览个数可选4-10个'>
                ...
                {moduleItem.code == '1100' && <FormItem label="首页预览个数" labelCol={{ span: 5 }} wrapperCol={{ span: 10 }} help='预览个数上线999个'>
                    {getFieldDecorator('previewNum', {
                        initialValue: moduleItem.previewNum,
                    })(
                        <InputNumber min={4} max={999} />
                    )}
                </FormItem>}
