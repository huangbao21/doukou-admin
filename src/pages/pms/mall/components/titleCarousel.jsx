 ...
 */
  handleChange = (index, name, value, arry) => {
    const { form, moduleItem,productList,groupList } = this.props
    arry[index][name] = value
    if(name == 'urlType'){
      //value=>1:商品 3：分组
      if (value == 1 && !arry[index].productId){
        arry[index].productId = productList[0].id
      }
      if (value == 3 && !arry[index].groupId) {
        arry[index].groupId = groupList[0].id
      }
    }
    this.setState({
      detailArray: arry
  ...
  render() {
    const { form: { getFieldDecorator }, moduleItem, detail, productList,groupList, afterSave } = this.props
    const { detailArray, clearImg } = this.state;
    // 商品列表
    let productOption = [];
    productOption = productList.map(product => {
        ...
        <div className={Styles.dashedBoard} key={item.id}>
          ...
          <FormItem label="链接类型" labelCol={{ span: 5 }} >
            <Radio.Group defaultValue={item.urlType} onChange={(event) => { this.handleChange(index, event.target.name, event.target.value, detailData) }} name="urlType">
              <Radio.Button value={1}>商品</Radio.Button>
          ...
          {item.urlType == 1 && <FormItem required label="选择商品" labelCol={{ span: 5 }} wrapperCol={{ span: 10 }}>
            <Select style={{ width: '100%' }} name="productId" defaultValue={String(item.productId || productList[0].id)} onChange={(event) => { this.handleChange(index, 'productId', event, detailData) }}
              showSearch
          ...
          {item.urlType == 3 && <FormItem required label="选择分组" labelCol={{ span: 5 }} wrapperCol={{ span: 10 }} >
