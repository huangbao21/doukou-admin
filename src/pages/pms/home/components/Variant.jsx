...
export default class Variant extends Component {
  ...
  addImg = (arry) => {
    const { moduleItem, allGroup } = this.props
    let obj = {
      groupId: allGroup[0].id,
      id: arry.length,
      url: '',
      urlType: 3,
      webUrl: '',
...
*/
  handleChange = (index, name, value, arry) => {
    const { form, moduleItem, allGroup } = this.props
  ...
  render() {
    const { form: { getFieldDecorator }, moduleItem, detail, allGroup, afterSave } = this.props
    const { detailArray, clearImg } = this.state;
    // 商品分组
    let groupOptions = [];
